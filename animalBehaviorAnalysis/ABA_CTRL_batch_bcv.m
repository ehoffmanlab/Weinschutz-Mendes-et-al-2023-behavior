function [cmd,mutant] = ABA_CTRL_batch(data_path,varargin)
% inputs
p = inputParser;
...
p.addParameter('reliability',true,@(x) islogical(x) || isnumeric(x));
...
if inputs.run && numel(cmd) > 0
    ...
    % reliability
    if inputs.reliability
        savePath = unique({cmd(arrayfun(@(x) isempty(x.err),cmd) & ~contains({cmd.projectName},'combined')).savePath});
        if ~isempty(savePath)
            if inputs.parallel
               parfor i=1:numel(savePath)
                  combinedReliability(savePath{i}); 
               end
            else
               for i=1:numel(savePath)
                   combinedReliability(savePath{i});
               end
            end
        end
